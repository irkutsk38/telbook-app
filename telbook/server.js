...
// Экспорт отчёта 1: Города
app.get('/export/cities', async (req, res) => {
  const [rows] = await db.execute(`
    SELECT city_name AS Город, phone_code AS Телефонный_код 
    FROM city 
    ORDER BY city_name
  `);

  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('Города по алфавиту');

  worksheet.columns = [
    { header: 'Город', key: 'Город', width: 25 },
    { header: 'Телефонный код', key: 'Телефонный_код', width: 15 }
  ];

  worksheet.addRows(rows);

  res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.setHeader('Content-Disposition', 'attachment; filename="goroda.xlsx"');
  await workbook.xlsx.write(res);
  res.end();
});

// Экспорт отчёта 2: Физлица
app.get('/export/persons', async (req, res) => {
  const [rows] = await db.execute(`
    SELECT 
      p.surname AS Фамилия,
      p.name AS Имя,
      COALESCE(p.patronymic, '') AS Отчество,
  ...
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('Физлица по фамилии');
  ...
  worksheet.columns = [
    ...
    { header: 'Город', key: 'Город', width: 20 }
  ];

  worksheet.addRows(rows);

  res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.setHeader('Content-Disposition', 'attachment; filename="phys_persons.xlsx"');
  await workbook.xlsx.write(res);
  res.end();
});

// Экспорт отчёта 3: Организации (города и рубрики)
app.get('/export/organizations', async (req, res) => {
  const [rows] = await db.execute(`
    SELECT 
      c.city_name AS Город,
      cat.category_name AS Рубрика,
      o.org_name AS Организация,
      GROUP_CONCAT(op.phone_number SEPARATOR ', ') AS Телефоны
    FROM organization o
    JOIN city c ON o.city_id = c.city_id
    JOIN category cat ON o.category_id = cat.category_id
    LEFT JOIN org_phone op ON o.org_id = op.org_id
  ...
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('Организации — города и рубрики');

  worksheet.columns = [
    { header: 'Город', key: 'Город', width: 20 },
    { header: 'Рубрика', key: 'Рубрика', width: 20 },
    { header: 'Организация', key: 'Организация', width: 30 },
    { header: 'Телефоны', key: 'Телефоны', width: 25 }
  ];

  worksheet.addRows(rows);

  res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.setHeader('Content-Disposition', 'attachment; filename="organizacii.xlsx"');
  await workbook.xlsx.write(res);
  res.end();
});

// Экспорт отчёта 4: Организации по рубрике
app.get('/export/by-rubric', async (req, res) => {
  const { category_name } = req.query;
  if (!category_name) {
    return res.status(400).send('Укажите рубрику через ?category_name=...');
  }
  ...
  const worksheet = workbook.addWorksheet(`Рубрика ${safeCategoryName}`);
  ...
  res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.setHeader('Content-Disposition', `attachment; filename="rubrika_${encodeURIComponent(category_name)}.xlsx"`);
  await workbook.xlsx.write(res);
  res.end();
});
app.listen(PORT, () => {
